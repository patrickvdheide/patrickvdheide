...
- patrickvdheide.nl
- 
- Detecteert automatisch welk Webflow-formulier is ingestuurd
- (#Contact-Formulier of #Offerte-Formulier) en verstuurt een
- passende bedankmail via Resend.
- 
...
- 1. Detecteer welk formulier is gebruikt
- 
- Primaire check  → formName (Webflow webhook)
- Fallback check  → aanwezigheid offerte-specifieke velden
- ───────────────────────────────────────── */
  const isOfferte =
...
- 5B. OFFERTEFORMULIER — veldnamen
- (conform id/name attributen in Webflow)
- ───────────────────────────────────────── */
  else {
...
- 1. Kies de juiste e-mail templates
- ───────────────────────────────────────── */
...
- 1. Verstuur via Resend
- ───────────────────────────────────────── */
  const apiKey = env.RESEND_API_KEY;

if (!apiKey) {
console.error(“RESEND_API_KEY is niet ingesteld als omgevingsvariabele.”);
return new Response(
...
body: JSON.stringify({
from: “Patrick vd Heide [hallo@patrickvdheide.nl](mailto:hallo@patrickvdheide.nl)”,
to: [email],
...
console.error(“Resend fout (bezoeker):”, err);
return new Response(
...
body: JSON.stringify({
from: “Website Formulier [hallo@patrickvdheide.nl](mailto:hallo@patrickvdheide.nl)”,
to: [internalTo],
...
- GEDEELDE CSS-STIJLEN
- Brand tokens patrickvdheide.nl
- Achtergrond : #080b10
- Kaart       : #0f1319
- Subtiel     : #13181f
- Divider     : #1e2530
...
- Muted       : #5a6a7e
- Accent      : #4c9eff
- Fonts       : Inter, Lato, system-ui (email-safe stack)
- ═══════════════════════════════════════════════════════════════ */
  const BASE_STYLES = `  *, *::before, *::after { box-sizing: border-box; }   body, table, td, a { -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; }   table, td { mso-table-lspace: 0pt; mso-table-rspace: 0pt; border-collapse: collapse; }   img { border: 0; height: auto; line-height: 100%; outline: none; text-decoration: none; }   a { color: inherit; text-decoration: none; }   body { margin: 0 !important; padding: 0 !important; background-color: #080b10; font-family: 'Inter', 'Lato', -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; -webkit-font-smoothing: antialiased;   }   .email-wrapper { width: 100%; background-color: #080b10; padding: 40px 16px; }   .email-card { max-width: 560px; margin: 0 auto; background-color: #0f1319; border-radius: 16px; border: 1px solid #1e2530; overflow: hidden;   }   .email-header { padding: 36px 40px 28px; text-align: center; border-bottom: 1px solid #1e2530; }   .header-label { display: inline-block; font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: #5a6a7e; margin-bottom: 12px;   }   .header-wordmark { font-size: 17px; font-weight: 800; color: #ffffff; letter-spacing: -0.03em; }   .header-wordmark span { color: #4c9eff; }   .email-hero { padding: 44px 40px 32px; text-align: center; }   .emoji-badge { font-size: 38px; margin-bottom: 18px; display: block; }   .hero-title { font-size: 26px; font-weight: 800; color: #ffffff; letter-spacing: -0.04em; line-height: 1.2; margin: 0 0 14px; }   .hero-subtitle { font-size: 15px; line-height: 1.7; color: #8a9bb0; margin: 0 auto; max-width: 420px; }   .hero-subtitle strong { color: #c9d5e3; font-weight: 600; }   .divider { height: 1px; background-color: #1e2530; margin: 0 40px; }   .summary-section { padding: 30px 40px; }   .summary-label { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: #5a6a7e; margin: 0 0 14px; }   .summary-card { background-color: #13181f; border-radius: 10px; border: 1px solid #1e2530; overflow: hidden; }   .summary-row { display: flex; padding: 12px 18px; border-bottom: 1px solid #1e2530; align-items: flex-start; gap: 12px; }   .summary-row:last-child { border-bottom: none; }   .summary-key { font-size: 11px; font-weight: 700; color: #5a6a7e; text-transform: uppercase; letter-spacing: 0.07em; width: 110px; flex-shrink: 0; padding-top: 2px; }   .summary-val { font-size: 14px; color: #c9d5e3; line-height: 1.55; flex: 1; word-break: break-word; }   .highlight-pill { display: inline-block; background-color: #1c2b40; color: #4c9eff; font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 100px; letter-spacing: 0.02em; }   .steps-card { background-color: #13181f; border-radius: 10px; border: 1px solid #1e2530; overflow: hidden; }   .step-row { display: flex; padding: 13px 18px; border-bottom: 1px solid #1e2530; align-items: flex-start; gap: 14px; }   .step-row:last-child { border-bottom: none; }   .step-num { font-size: 13px; font-weight: 800; color: #4c9eff; width: 20px; flex-shrink: 0; padding-top: 1px; }   .step-text { font-size: 14px; color: #8a9bb0; line-height: 1.55; flex: 1; }   .step-text strong { color: #c9d5e3; font-weight: 600; }   .cta-section { padding: 8px 40px 40px; text-align: center; }   .cta-text { font-size: 15px; color: #8a9bb0; margin: 0 0 22px; line-height: 1.65; }   .cta-text strong { color: #c9d5e3; font-weight: 600; }   .cta-btn { display: inline-block; background-color: #ffffff; color: #0c0f16 !important; font-size: 14px; font-weight: 700; letter-spacing: -0.01em; padding: 14px 30px; border-radius: 100px;   }   .email-footer { padding: 26px 40px; border-top: 1px solid #1e2530; text-align: center; }   .footer-signature { font-size: 13px; font-weight: 700; color: #5a6a7e; margin: 0 0 6px; }   .footer-text { font-size: 11px; color: #3d4d5e; line-height: 1.8; margin: 0; }   .footer-text a { color: #5a6a7e; }   @media only screen and (max-width: 600px) { .email-wrapper   { padding: 20px 12px !important; } .email-header    { padding: 24px 22px 22px !important; } .email-hero      { padding: 32px 22px 26px !important; } .hero-title      { font-size: 21px !important; } .divider         { margin: 0 22px !important; } .summary-section { padding: 22px 22px !important; } .cta-section     { padding: 8px 22px 30px !important; } .email-footer    { padding: 22px 22px !important; } .summary-row     { flex-direction: column; gap: 3px; } .step-row        { flex-direction: row; } .summary-key     { width: auto !important; } .cta-btn         { display: block !important; }   }`;

/* ═══════════════════════════════════════════════════════════════

- TEMPLATE 1 — BEDANKMAIL CONTACTFORMULIER
- Toon: warm, persoonlijk, laagdrempelig
- ═══════════════════════════════════════════════════════════════ */
...
<body>
  <div style="display:none;max-height:0;overflow:hidden;">
    Hey ${voornaam}, ik heb je bericht ontvangen en kom er zo snel mogelijk op terug. &#8199;&#65279;&#847;&#8199;&#65279;&#847;&#8199;&#65279;
  </div>
...
- Extra velden : pakket, budget
- Toon         : professioneler, verwachting wekken, volgende stap
- ═══════════════════════════════════════════════════════════════ */
...
<head>
  ...
  <meta name="color-scheme" content="dark"/>
  <meta name="supported-color-schemes" content="dark"/>
  <title>Hey ${voornaam} — jouw offerte-aanvraag is ontvangen 🙌</title>
  <style>${BASE_STYLES}</style>
</head>
<body>
  <div style="display:none;max-height:0;overflow:hidden;">
    Jouw offerte-aanvraag is ontvangen, ${voornaam}. Ik neem binnen één werkdag contact op. &#8199;&#65279;&#847;&#8199;&#65279;&#847;&#8199;&#65279;
  </div>
  <div class="email-wrapper">
    <div class="email-card">
...
```
  ...
  <div class="cta-section">
    <p class="cta-text" style="margin-top:28px;">
      Wil je alvast een gesprek inplannen?
      <strong>Dat kan direct.</strong>
    </p>
    <a href="https://meetings-eu1.hubspot.com/patrick-van-der-heide?uuid=0e32d639-d879-48e4-bd2d-d032fb386119" class="cta-btn">
...
- INTERNE NOTIFICATIE — CONTACTFORMULIER
- ═══════════════════════════════════════════════════════════════ */
...
- INTERNE NOTIFICATIE — OFFERTEFORMULIER
- ═══════════════════════════════════════════════════════════════ */
...
/* ─── Gedeelde interne mail builder ──────────────────────────── */
function buildInternalEmail({ badge, title, meta, rows }) {
const rowsHtml = rows.map(r => ` <div style="display:flex;border-bottom:1px solid #1e2530;padding:11px 0;align-items:flex-start;gap:12px;"> <span style="font-size:11px;font-weight:700;color:#5a6a7e;text-transform:uppercase;letter-spacing:.06em;width:80px;flex-shrink:0;padding-top:2px;">${r.key}</span> <span style="font-size:14px;color:#c9d5e3;flex:1;word-break:break-word;line-height:1.55;">${r.val}</span> </div>`).join(””);
